...
#define IssueBookPath "privat/issued.txt"
#define DataPath "privat/data.txt"
#define BookFilePath "privat/books.txt"
...
void *authenticate_user(void *arg)
{
    ...
    while (1)
    {
        ...
        while (fgets(line, sizeof(line), file))
        {
            ...
            token = strtok(NULL, "|");
            if (token)
                strncpy(s.password, token, sizeof(s.password));
            token = strtok(NULL, "|");
            if (token)
                strncpy(s.id, token, sizeof(s.id));
...
void *issueBook(void *arg)
{
    ...
    while (!book_found)
    {
        ...
        FILE *book_fp = fopen(BookFilePath, "r");
        if (!book_fp)
        {
            perror("Failed to open books.txt");
            return NULL;
        ...
        else
        {
            ...
            FILE *rewrite_fp = fopen(BookFilePath, "w");
            for (int i = 0; i < count; i++)
            {
...
void *returnBook(void *arg)
{
    ...
    if (issued_found)
    {
        ...
        FILE *out = fopen(IssueBookPath, "w");
        if (!out)
        {
        ...
        FILE *book_out = fopen(BookFilePath, "a");
        if (!book_out)
        {
            perror("Error opening books.txt");
            return NULL;
                ...
                if (*(int *)authResult == 1)
                {
                    ...
                    while (continueMenu)
                    {
                        ...
                        case 9:
                            ...
                            break;
                        case 0:
                            continueMenu = 0; // exit from authenticated menu
                            break;
                        default:
                            printf("Wrong Input!\n");
                            break;
                    ...
                    tryAgain = 0; // Exit the login loop after successful session
                }
                else
                {
                    char response;
                ...
                if (*(int *)authResult == 1)
                {
                    ...
                    while (inMenu)
                    {
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            pthread_create(&threads[7], NULL, display, NULL);
                            pthread_join(threads[7], NULL);
                            break;
                        case 3:
